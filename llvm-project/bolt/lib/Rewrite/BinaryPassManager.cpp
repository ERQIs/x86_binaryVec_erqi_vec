...
static cl::opt<bool>
    PrintZeroIdiom("erqi-vec",
                   cl::desc("erqi vectorization"),
                   cl::cat(BoltOptCategory));

static cl::opt<bool>
    EmitMachineCodeInst("emit-mci",
                   cl::desc("dump machine code"),
                   cl::cat(BoltOptCategory));
...
Error BinaryFunctionPassManager::runAllPasses(BinaryContext &BC) {
  ...
  Manager.registerPass(std::make_unique<ErqiVec>(PrintZeroIdiom));
  // Manager.registerPass(std::make_unique<EmitMCI>(EmitMachineCodeInst));
