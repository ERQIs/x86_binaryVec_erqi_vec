...
//   erqi-vec starts here *********************************************************************
  //************************************************************************************* */
  //************************************************************************************* */

  /// 返回第 `ArgNo` 个 64 位通用寄存器
  MCPhysReg getGPR64(unsigned ArgNo) const override {
    ...
    return (ArgNo < sizeof(GPR64) / sizeof(MCPhysReg)) ? GPR64[ArgNo] : 0;
  }

  /// 返回第 `ArgNo` 个 32 位通用寄存器
  MCPhysReg getGPR32(unsigned ArgNo) const override {
    static const MCPhysReg GPR32[] = {
        X86::EAX, X86::EBX, X86::ECX, X86::EDX,
        X86::ESI, X86::EDI, X86::EBP, X86::ESP,
        X86::R8D, X86::R9D, X86::R10D, X86::R11D,
        X86::R12D, X86::R13D, X86::R14D, X86::R15D
    };
    return (ArgNo < sizeof(GPR32) / sizeof(MCPhysReg)) ? GPR32[ArgNo] : 0;
  }

  /// 返回第 `ArgNo` 个 8 位通用寄存器
  MCPhysReg getGPR8(unsigned ArgNo) const override {
    static const MCPhysReg GPR8[] = {
        X86::AL, X86::BL, X86::CL, X86::DL,
        X86::SIL, X86::DIL, X86::BPL, X86::SPL,
        X86::R8B, X86::R9B, X86::R10B, X86::R11B,
        X86::R12B, X86::R13B, X86::R14B, X86::R15B
    };
    return (ArgNo < sizeof(GPR8) / sizeof(MCPhysReg)) ? GPR8[ArgNo] : 0;
  }

  /// 返回第 `ArgNo` 个 XMM 寄存器
  MCPhysReg getXMM(unsigned ArgNo) const override {
    static const MCPhysReg XMMRegs[] = {
        X86::XMM0, X86::XMM1, X86::XMM2, X86::XMM3,
        X86::XMM4, X86::XMM5, X86::XMM6, X86::XMM7,
        X86::XMM8, X86::XMM9, X86::XMM10, X86::XMM11,
        X86::XMM12, X86::XMM13, X86::XMM14, X86::XMM15
    };
    return (ArgNo < sizeof(XMMRegs) / sizeof(MCPhysReg)) ? XMMRegs[ArgNo] : 0;
  }

  /// 返回第 `ArgNo` 个 YMM 寄存器
  MCPhysReg getYMM(unsigned ArgNo) const override {
    static const MCPhysReg YMMRegs[] = {
    ...
    return (ArgNo < sizeof(YMMRegs) / sizeof(MCPhysReg)) ? YMMRegs[ArgNo] : 0;
  }

  /// 返回第 `ArgNo` 个 ZMM 寄存器
  MCPhysReg getZMM(unsigned ArgNo) const override {
    static const MCPhysReg ZMMRegs[] = {
    ...
    return (ArgNo < sizeof(ZMMRegs) / sizeof(MCPhysReg)) ? ZMMRegs[ArgNo] : 0;
  }

    // Create a MOV from memory to ZMM register (rm)
  void createZmmMovRm(MCInst &Inst, MCPhysReg srcGPReg,MCPhysReg dstVReg) const override {
    Inst.setOpcode(X86::VMOVDQU64Zrm); // MOVSS for single-precision (use MOVSDrm for double)
    Inst.clear();
    // 添加操作数
      // 向量寄存器
    Inst.addOperand(MCOperand::createReg(dstVReg));
    // 内存GPR操作数
    Inst.addOperand(MCOperand::createReg(srcGPReg));  // 基址寄存器  
    Inst.addOperand(MCOperand::createImm(1));         // 缩放因子 (默认为1)
    Inst.addOperand(MCOperand::createReg(0));  // 索引寄存器（默认0）
    Inst.addOperand(MCOperand::createImm(0));    // 偏移量 （默认0）
    Inst.addOperand(MCOperand::createReg(0));   // 占位的寄存器
  }

  // Create a MOV from ZMM register to memory (mr)
  // 	vmovdqu64	%zmm0,	(%rdx)
  void createZmmMovMr(MCInst &Inst, MCPhysReg srcVReg, MCPhysReg dstGPReg) const override {
    Inst.clear();
    Inst.setOpcode(X86::VMOVDQU64Zmr); // MOVSS for single-precision (use MOVSDmr for double)
    // 添加操作数
    // 内存GPR操作数
    Inst.addOperand(MCOperand::createReg(dstGPReg));  // 基址寄存器  
    Inst.addOperand(MCOperand::createImm(1));         // 缩放因子 (默认为1)
    Inst.addOperand(MCOperand::createReg(0));  // 索引寄存器（默认0）
    Inst.addOperand(MCOperand::createImm(0));    // 偏移量 （默认0）
    Inst.addOperand(MCOperand::createReg(0));   // 占位的寄存器
    // 向量寄存器
    Inst.addOperand(MCOperand::createReg(srcVReg));
  }

  // Create an ADD instruction for XMM registers
  void createZmmAdd(MCInst &Inst, MCPhysReg DstReg, MCPhysReg SrcReg1, MCPhysReg SrcReg2) const override {
    Inst.setOpcode(X86::VPADDDZrr); // ADDSS for single-precision (use ADDSDrr for double)
    Inst.clear();
    Inst.addOperand(MCOperand::createReg(DstReg)); // Destination register
    Inst.addOperand(MCOperand::createReg(SrcReg1));  // Source register
    Inst.addOperand(MCOperand::createReg(SrcReg2));  // Source register
  }

  // Create an ADD instruction for XMM registers
  void createZmmAddb(MCInst &Inst, MCPhysReg DstReg, MCPhysReg SrcReg1, MCPhysReg SrcReg2) const override {
    Inst.setOpcode(X86::VPADDBZrr); // ADDSS for single-precision (use ADDSDrr for double)
    Inst.clear();
    Inst.addOperand(MCOperand::createReg(DstReg)); // Destination register
    Inst.addOperand(MCOperand::createReg(SrcReg1));  // Source register
    Inst.addOperand(MCOperand::createReg(SrcReg2));  // Source register
  ...
  // Create a SUB instruction for XMM registers
  void createZmmSub(MCInst &Inst, MCPhysReg DstReg, MCPhysReg SrcReg1, MCPhysReg SrcReg2) const override {
    Inst.setOpcode(X86::VPSUBDZrr); // SUBSS for single-precision (use SUBSDrr for double)
    Inst.clear();
    Inst.addOperand(MCOperand::createReg(DstReg)); // Destination register
    Inst.addOperand(MCOperand::createReg(SrcReg1));  // Source register
    Inst.addOperand(MCOperand::createReg(SrcReg2));  // Source register
  ...
  // Create a MUL instruction for XMM registers
  void createZmmMul(MCInst &Inst, MCPhysReg DstReg, MCPhysReg SrcReg1, MCPhysReg SrcReg2) const override {
    Inst.setOpcode(X86::VPMULLDZrr); // MULSS for single-precision (use MULSDrr for double)
    Inst.clear();
    Inst.addOperand(MCOperand::createReg(DstReg)); // Destination register
    Inst.addOperand(MCOperand::createReg(SrcReg1));  // Source register
    Inst.addOperand(MCOperand::createReg(SrcReg2));  // Source register
  ...
  void createGPRinc(MCInst &Inst, MCPhysReg GPR, int step) const override {
    Inst.setOpcode(X86::ADD64ri8); // MULSS for single-precision (use MULSDrr for double)
    Inst.clear();
    Inst.addOperand(MCOperand::createReg(GPR));
    Inst.addOperand(MCOperand::createReg(GPR)); 
    Inst.addOperand(MCOperand::createImm(step)); 
  }

  void createADD64rr(MCInst &Inst, MCPhysReg src, MCPhysReg dst) const override {
    Inst.setOpcode(X86::ADD64rr); // MULSS for single-precision (use MULSDrr for double)
    Inst.clear();
  ...
  void createLoadFromPtrReg(MCInst &Inst, MCPhysReg ptr, MCPhysReg dst) const override {
    Inst.setOpcode(X86::MOV64rm); // MULSS for single-precision (use MULSDrr for double)
    Inst.clear();
    Inst.addOperand(MCOperand::createReg(dst));
    Inst.addOperand(MCOperand::createReg(ptr)); 
  ...
  void createGPRMulImm(MCInst &Inst, MCPhysReg gpr, int imm) const override {
    Inst.setOpcode(X86::IMUL64rri32);
    Inst.clear();
    Inst.addOperand(MCOperand::createReg(gpr));
    Inst.addOperand(MCOperand::createReg(gpr)); 
    Inst.addOperand(MCOperand::createImm(imm)); 
  ...
  void createMovImmToGPR(MCInst &Inst, MCPhysReg gpr, int imm) const override {
  ...
  void createBroadCast32ToZmm (MCInst &Inst, MCPhysReg gpr, MCPhysReg zmm) const override {
    Inst.setOpcode(X86::VPBROADCASTDrZrr);
    Inst.clear();
    Inst.addOperand(MCOperand::createReg(zmm));
    Inst.addOperand(MCOperand::createReg(gpr)); 
  }

  void createZmmMov(MCInst &Inst, MCPhysReg src, MCPhysReg dst) const override {
    Inst.setOpcode(X86::VMOVDQA64Zrr);
    Inst.clear();
    Inst.addOperand(MCOperand::createReg(dst));
    Inst.addOperand(MCOperand::createReg(src));
  }

  void createLeaAsAdd(MCInst &Inst, MCPhysReg srcReg64,MCOperand expOpr, MCPhysReg dstReg64) const override {
    Inst.setOpcode(X86::LEA64r); // MULSS for single-precision (use MULSDrr for double)
    Inst.clear();

    Inst.addOperand(MCOperand::createReg(dstReg64));

    Inst.addOperand(MCOperand::createReg(srcReg64)); 
    Inst.addOperand(MCOperand::createImm(1));
    Inst.addOperand(MCOperand::createReg(0));
    Inst.addOperand(expOpr);
    Inst.addOperand(MCOperand::createReg(0));
  }

  void createMovqrr(MCInst &Inst, MCPhysReg DstReg, MCPhysReg SrcReg) const override {
    Inst.setOpcode(X86::MOV64rr); // MULSS for single-precision (use MULSDrr for double)
    Inst.clear();
    Inst.addOperand(MCOperand::createReg(DstReg)); // Destination register
    Inst.addOperand(MCOperand::createReg(SrcReg));  // Source register
  }

  bool simLoopLifter (std::vector<MCInst> &MCInsts, std::vector<std::shared_ptr< simLoop::SimLoopInst>> &simLoopInsts, std::map<int, MCOperand> &SimLoopExpOps) const override {
    static int expr_id = 1;

    auto extractPtr = [&simLoopInsts,&SimLoopExpOps, this](MCOperand base, MCOperand scale, MCOperand index, MCOperand offset) -> std::shared_ptr<simLoop::RegOperand> {
      std::shared_ptr<simLoop::SimLoopInst> sli;
      if ((index.isReg() && index.getReg() == 0)) {
        // 没有索引寄存器
        if(offset.isImm() && offset.getImm() == 0) {
          // 没有偏移量
          // (%rax)
          return std::make_shared<simLoop::RegOperand>(
              base.getReg(), 
              "pReg_" + std::to_string(base.getReg()),
              8
            );
        } else if (offset.isExpr()) {
        // 偏移量是exp   "vec1/1"(%rsi)
          int expImm = expr_id ++;
          SimLoopExpOps.emplace(expImm, offset);
          std::shared_ptr<simLoop::RegOperand> tmp = std::make_shared<simLoop::RegOperand>(0, "tmp", 8);
          sli = std::make_shared<simLoop::BinaryInst> (
            simLoop::InstType::Add,
            std::make_shared<simLoop::ImmOperand>(expImm, true),
            std::make_shared<simLoop::RegOperand>(
              base.getReg(), 
              "pReg_" + std::to_string(base.getReg()),
              8
            ),
          ...
          simLoopInsts.push_back(sli);
          return tmp;
        } else if (offset.isImm()) {
          int imm = offset.getImm();
          std::shared_ptr<simLoop::RegOperand> tmp = std::make_shared<simLoop::RegOperand>(0, "tmp", 8);
          sli = std::make_shared<simLoop::BinaryInst> (
            simLoop::InstType::Add,
            std::make_shared<simLoop::ImmOperand>(imm, false),
            std::make_shared<simLoop::RegOperand>(
              base.getReg(), 
              "pReg_" + std::to_string(base.getReg()),
              8
      ...
      } else if ((index.isReg() && index.getReg() != 0) && 
                 (scale.isImm() && scale.getImm() != 0)) {
        std::shared_ptr<simLoop::RegOperand> tmp = std::make_shared<simLoop::RegOperand>(0, "tmp", 8);

        std::shared_ptr<simLoop::RegOperand> indexReg = std::make_shared<simLoop::RegOperand>(
              index.getReg(), 
              "pReg_" + std::to_string(index.getReg()),
              8
            );

          sli = std::make_shared<simLoop::BinaryInst> (
            simLoop::InstType::Mul,
            indexReg,
            std::make_shared<simLoop::ImmOperand>(scale.getImm(), false),
            tmp
          );
        simLoopInsts.push_back(sli);
    ...
    for (auto MCInst : MCInsts) {
      std::shared_ptr<simLoop::SimLoopInst> sli;
      switch (MCInst.getOpcode()) {
        case X86::MOV32rm : {
          MCOperand dstMCR = MCInst.getOperand(0);
          std::shared_ptr<simLoop::RegOperand> dst = 
            std::make_shared<simLoop::RegOperand>(
              dstMCR.getReg(),
              "pReg_" + std::to_string(dstMCR.getReg()),
              4
            );
          std::shared_ptr<simLoop::RegOperand> src = extractPtr(MCInst.getOperand(1), MCInst.getOperand(2), MCInst.getOperand(3), MCInst.getOperand(4));
          sli =  std::make_shared<simLoop::LoadInst>(src, dst);
          simLoopInsts.push_back(sli);
          //loadMemoryTo(dst, MCInst.getOperand(1), MCInst.getOperand(2), MCInst.getOperand(3), MCInst.getOperand(4));
          break;
        }
        case X86::ADD32rm : {
          MCOperand dstMCR = MCInst.getOperand(0);
          ...
          std::shared_ptr<simLoop::RegOperand> tmp = std::make_shared<simLoop::RegOperand>(0,"tmp",4);

          std::shared_ptr<simLoop::RegOperand> src = extractPtr(MCInst.getOperand(2), MCInst.getOperand(3), MCInst.getOperand(4), MCInst.getOperand(5));
          sli =  std::make_shared<simLoop::LoadInst>(src, tmp);
          simLoopInsts.push_back(sli);
          // loadMemoryTo(tmp, MCInst.getOperand(2), MCInst.getOperand(3), MCInst.getOperand(4), MCInst.getOperand(5));
          sli = std::make_shared<simLoop::BinaryInst>(simLoop::InstType::Add, dst, tmp, dst);
          simLoopInsts.push_back(sli);
          break;
        }
        case X86::IMUL32rm : {
          ...
          sli =  std::make_shared<simLoop::LoadInst>(src, tmp);
          simLoopInsts.push_back(sli);
          sli = std::make_shared<simLoop::BinaryInst>(simLoop::InstType::Mul, dst, tmp, dst);
          simLoopInsts.push_back(sli);
          break;
        }
        case X86::SUB32rm : {
          ...
          sli =  std::make_shared<simLoop::LoadInst>(src, tmp);
          simLoopInsts.push_back(sli);
          sli = std::make_shared<simLoop::BinaryInst>(simLoop::InstType::Sub, dst, tmp, dst);
          simLoopInsts.push_back(sli);
          break;
        }
        case X86::MOV32mr: {
          std::shared_ptr<simLoop::RegOperand> dst = extractPtr(MCInst.getOperand(0), MCInst.getOperand(1), MCInst.getOperand(2), MCInst.getOperand(3));
          MCOperand srcMCR = MCInst.getOperand(5);
          std::shared_ptr<simLoop::RegOperand> src = 
            std::make_shared<simLoop::RegOperand>(
              srcMCR.getReg(),
              "pReg_" + std::to_string(srcMCR.getReg()),
              4
            );
          sli =  std::make_shared<simLoop::StoreInst>(src, dst);
          simLoopInsts.push_back(sli);
          break;
        }
        case X86::ADD64ri8: {
          MCOperand MCROpr = MCInst.getOperand(0);
          int imm = MCInst.getOperand(2).getImm();
          std::shared_ptr<simLoop::RegOperand> regOpr = 
            std::make_shared<simLoop::RegOperand>(
              MCROpr.getReg(),
              "pReg_" + std::to_string(MCROpr.getReg()),
              4
            );
          sli = std::make_shared<simLoop::BinaryInst>(
            simLoop::InstType::Add, 
            regOpr, 
            std::make_shared<simLoop::ImmOperand>(imm), 
            regOpr);
          simLoopInsts.push_back(sli);
          break;
        }
        ...
        case X86::MOV8rm: {
          MCOperand dstMCR = MCInst.getOperand(0);
          std::shared_ptr<simLoop::RegOperand> ptr = extractPtr(MCInst.getOperand(1), MCInst.getOperand(2), MCInst.getOperand(3), MCInst.getOperand(4));
          sli = std::make_shared<simLoop::LoadInst>(
            ptr,
            std::make_shared<simLoop::RegOperand>(
              dstMCR.getReg(),
              "pReg_" + std::to_string(dstMCR.getReg()),
        ...
        case X86::ADD8rm: {
          MCOperand dstMCR = MCInst.getOperand(0);
          std::shared_ptr<simLoop::RegOperand> tmp = std::make_shared<simLoop::RegOperand>(-1,"tmp0",8);
          std::shared_ptr<simLoop::RegOperand> ptr = extractPtr(MCInst.getOperand(2), MCInst.getOperand(3), MCInst.getOperand(4), MCInst.getOperand(5));

          sli = std::make_shared<simLoop::LoadInst>(ptr, tmp);
          simLoopInsts.push_back(sli);

          std::shared_ptr<simLoop::RegOperand> dst = std::make_shared<simLoop::RegOperand>(
              dstMCR.getReg(),
              "pReg_" + std::to_string(dstMCR.getReg()),
              1
          );
        ...
        case X86::LEA64r: {
          std::shared_ptr<simLoop::RegOperand> val = extractPtr(MCInst.getOperand(1), MCInst.getOperand(2), MCInst.getOperand(3), MCInst.getOperand(4));
          MCOperand dstMCR = MCInst.getOperand(0);
          std::shared_ptr<simLoop::RegOperand> dst = 
            std::make_shared<simLoop::RegOperand>(
              dstMCR.getReg(),
          ...
          sli = std::make_shared<simLoop::BinaryInst>(
            simLoop::InstType::Add,
            std::make_shared<simLoop::ImmOperand>(0, false),
            val,
            dst);
          simLoopInsts.push_back(sli);
          break;
        ...
        case X86::MOV32mi: {
          int imm = MCInst.getOperand(5).getImm();
          std::shared_ptr<simLoop::RegOperand> dst = extractPtr(MCInst.getOperand(0), MCInst.getOperand(1), MCInst.getOperand(2), MCInst.getOperand(3));
          sli =  std::make_shared<simLoop::StoreInst>(
              std::make_shared<simLoop::ImmOperand>(imm, false), 
              dst
              );
          simLoopInsts.push_back(sli);
        ...
        case X86::ADD64rr: {
          ...
          MCOperand srcMCR = MCInst.getOperand(2);
          std::shared_ptr<simLoop::RegOperand> src = 
            std::make_shared<simLoop::RegOperand>(
              srcMCR.getReg(),
              "pReg_" + std::to_string(srcMCR.getReg()),
        ...
        case X86::ADD64mi8: {
          int imm = MCInst.getOperand(5).getImm();
          std::shared_ptr<simLoop::RegOperand> ptr = extractPtr(MCInst.getOperand(0), MCInst.getOperand(1), MCInst.getOperand(2), MCInst.getOperand(3));

          std::shared_ptr<simLoop::RegOperand> val = std::make_shared<simLoop::RegOperand>(-1, "tmp0", 8);

          sli =  std::make_shared<simLoop::LoadInst>(ptr, val);
          simLoopInsts.push_back(sli);
          ...
          sli = std::make_shared<simLoop::StoreInst>(val, ptr);
          simLoopInsts.push_back(sli);
          break;
        }
        default : {
          errs() << "unimplemented inst\n";
          break;
        }
